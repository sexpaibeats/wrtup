#!/bin/sh
# ==============================================================================
# Поддержк OpenWrt 24 / 25+ / GL.iNet
...
printf "${C}[?]${W} Применить сетевые твики (Sysctl TCP, Wi-Fi AQL)? (y/N)

Подробнее о твиках:
Твики ядра и TCP (Sysctl):
 ⁠net.ipv4.tcp_fastopen=3⁠ — включает TCP Fast Open. Позволяет отправлять данные уже в самом первом пакете (SYN) при установке соединения. Ускоряет открытие страниц, снижает задержки (особенно полезно при работе с прокси/VLESS).
 ⁠net.ipv4.tcp_slow_start_after_idle=0⁠ — отключает «медленный старт» после простоя. Если вы смотрели видео, поставили на паузу и через минуту сняли, соединение не будет заново плавно разгоняться, а сразу продолжит качать на максимальной скорости.
...
Твики Wi-Fi (AQL — Airtime Queue Limits):
 ...
 Если кто-то из домашних начинает качать тяжелый файл по Wi-Fi, радиоэфир забивается. Без AQL пинг в играх у других устройств по Wi-Fi может скакать до 100–300 мс.
 Эти команды (⁠aql_txq_limit⁠, ⁠fq_limit⁠) аппаратно включают алгоритм Fair Queuing (честная очередь) на уровне радиодрайвера ⁠mac80211⁠ (который используется во Flint 2 и Redmi). Пинг остается стабильным даже при забитом эфире. : "
read APPLY_TWEAKS
