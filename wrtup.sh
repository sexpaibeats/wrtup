...
# ============================================================================
#  
#  Универсальный скрипт первой настройки роутера на OpenWrt / ImmortalWrt
#  и прошивках GL.iNet (поддержка apk и opkg, 24.10 / 25.12+ / 4.5+).
#
#  Проверено на: GL.iNet Flint 2 (GL-MT6000), Xiaomi/Redmi AX6S (ImmortalWrt).
#  Возможно будет работать на других устройствах OpenWrt/ImmortalWrt/Gl.inet.
#
...
#  повторить/пересоздать. Некоторые фичи подтверждаются вопросом
...
show_script_description() {
  section "Что установит/настроит этот скрипт?"
  cat <<'EOF'
  • добавление репозиториев OpenWrt и ImmortalWrt
  • русский язык для GL.iNet и LuCI
  • отключение принудительной VPN-маршрутизации GL.iNet
  • forkop и отдельная сеть vpnnet (192.168.20.0/24)
  • MiniUPnP для автооткрытия портов
  • zram
  • тема LuCI Footstrap
  • CPU governor "performance"
  • сетевые твики
  • полезные пакеты
  • часовой пояс по городу
EOF
}
...
cleanup_broken_repos() {
  if [ "$PKG" = "apk" ]; then
    ...
    # Обязательно создаём файл, чтобы LuCI не выдавал NoDataError
    [ -f /etc/apk/repositories ] || touch /etc/apk/repositories
    
    # Удаляем любые ссылки http/https, которые не оканчиваются на .adb
    for f in /etc/apk/repositories /etc/apk/repositories.d/*.list; do
      if [ -f "$f" ]; then
        sed -i -e '/^http/!b' -e '/\.adb$/b' -e 'd' "$f" 2>/dev/null
      fi
    done
  fi
}
